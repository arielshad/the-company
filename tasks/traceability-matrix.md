...
| T4.2 Notion connector (read) — first real source | FR-2.1, FR-2.5 | S | contract + conformance + integration | _pending_ |
| T4.3 Google Drive connector (read) | FR-2.1, FR-2.5 | S | contract + conformance | _pending_ |
| T4.4 Zoom connector (real API + webhook → ingest + trigger) | FR-2.1, FR-6.3 | S | contract + conformance; feeds flagship e2e | _pending_ |
| T4.5 Outbound Slack notify (real, gated, idempotent) | FR-9.1 | S | contract (no double-send on replay) | _pending_ |
...
| T5.1 Real MCP server transport + external trust boundary | FR-7.1, FR-7.2, FR-7.4, FR-7.5, NFR-1 | F | contract + e2e + `/security-review` | _pending_ |
