...
describe("SqliteAuthz — same ReBAC semantics as in-memory", () => {
  it("org admin is brain writer + reader; member is reader only", () => {
    const fga = seeded();
  ...
  it("derives memory_object viewer from brain reader", () => {
    const fga = seeded();
  ...
  it("resolves userset tuples (team#member → brain reader)", () => {
...
describe("SqliteAuthz — durability", () => {
  it("persists tuples across a reopen (process restart)", () => {
    const dir = mkdtempSync(join(tmpdir(), "fga-"));
    tmpDirs.push(dir);
    const path = join(dir, "authz.db");

    const first = seeded(path);
    expect(first.check("user:alice", "writer", "brain:acme")).toBe(true);
    first.close();

    // reopen a fresh engine against the same file — relations survive
    const reopened = new SqliteAuthz(path);
    expect(reopened.check("user:alice", "writer", "brain:acme")).toBe(true);
    expect(reopened.check("user:bob", "reader", "brain:acme")).toBe(true);
    reopened.close();
  });
});
