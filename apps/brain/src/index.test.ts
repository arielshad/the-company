...
describe("BrainService — permission-aware search (FR-3.5)", () => {
  ...
  it("hides a restricted document from an unauthorized user", async () => {
    const hits = await brain.search(bob, { orgId: ORG, query: "Q3 strategy" });
    expect(hits.find((h) => h.title === "Q3 Strategy")).toBeUndefined();
  });

  it("shows the restricted document to an authorized user with provenance", async () => {
    const hits = await brain.search(alice, { orgId: ORG, query: "Q3 strategy" });
  ...
  it("denies a non-member entirely (OpenFGA)", async () => {
    const hits = await brain.search(carol, { orgId: ORG, query: "strategy" });
    expect(hits).toHaveLength(0);
  });

  it("returns public docs to any member", async () => {
    const hits = await brain.search(bob, { orgId: ORG, query: "onboarding strategy" });
...
describe("BrainService — memory write (FR-3.6/3.7)", () => {
  ...
  it("writes when authorized and policy satisfied", async () => {
    const audit = new InMemoryAudit();
    const brain = new BrainService(seedAcme(), audit);
    const r = await brain.writeMemory(alice, base, policy);
  ...
  it("rejects an unauthorized writer", async () => {
    const brain = new BrainService(seedAcme());
    const r = await brain.writeMemory(bob, base, policy); // bob is member, not writer
    expect(r).toMatchObject({ status: "rejected", reason: "not_authorized" });
  });

  it("routes low-confidence writes to approval", async () => {
    const brain = new BrainService(seedAcme());
    const r = await brain.writeMemory(alice, { ...base, confidence: 0.6 }, policy);
    expect(r.status).toBe("needs_approval");
  });

  it("rejects disallowed types and sub-threshold confidence", async () => {
    const brain = new BrainService(seedAcme());
    expect((await brain.writeMemory(alice, { ...base, type: "risk" }, policy)).status).toBe("rejected");
    expect((await brain.writeMemory(alice, { ...base, confidence: 0.1 }, policy)).status).toBe("rejected");
  });

  it("supersede + lineage work", async () => {
    const brain = new BrainService(seedAcme());
    ...
    const hits = await brain.search(alice, { orgId: ORG, query: "prioritize SSO" });
